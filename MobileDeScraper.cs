...
namespace AutoDataScraper
{
    ...
    {
        public MobileDeScraper(string initLink, string folder) : base(initLink, folder)
        {
        ...
        {
            return int.Parse(Regex.Match(m_CurrentLink, @"pageNumber=(\d+)").Groups[1].Value);
        }

        public override int GetLastPage(string html)
        {
            var pattern = @"Go to page (\d+)";
            ...
            {
                page = Math.Max(page, int.Parse(matches[i].Groups[1].Value));
            }
